...
def cut_one_image_random(input_path, output_path, file_name):
    ...
    imgw = img.shape[1]
    imgh = img.shape[0]
    if (imgw<224 or imgh<384):
        print file_name, "file too small"
        return
    img = cv2.resize(img,(rectWidth,rectWidth))
    tName = "%s/%s.bmp" %(output_path, file_name.split('.')[0])
...
def cut_image_patches(source_real, source_CG, patch_dir='..'): 
    ...
    if not os.path.exists(patch_dir):
        os.mkdir(patch_dir)
        os.mkdir(patch_dir + '/CG/')
        os.mkdir(patch_dir + '/PG/')
    ...
    for i in range(number_real):
        ...
        cut_one_image_random(source_real, patch_dir + '/PG/', image_real[i])
        cut_one_image_random(source_CG, patch_dir + '/CG/', image_CG[i])
